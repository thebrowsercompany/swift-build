...
# 3. Dates the runner names with the date from the machine_image version field
...
def main():
    ...
    next_release_match = re.search(
        release_header_pattern, text[after_main_header_idx:], re.MULTILINE
    )
    ...
    if next_release_match:
        ...
        if line_start_before_match == -1:
            ...
            main_block_end_idx = (
                line_start_before_match + 1
            )  # +1 to point after the newline
    else:
    ...
    # Process each runner individually to extract its machine_image date
    # Find all runner start positions (each starts with "  - name:")
    runner_starts = list(re.finditer(r"^  - name:", main_block_body, re.MULTILINE))

    if not runner_starts:
        print("Error: Could not find any runner definitions in main block.")
        sys.exit(1)

    processed_runners = []
    machine_image_pattern = r"/versions/(\d{4})\.(\d{2})\.(\d{2})"
    label_pattern = re.compile(r"(?P<indent>\s*)-\s+(?P<value>cirun-[^\s#]+)")

    for i, match in enumerate(runner_starts):
        # Get the start position of this runner
        runner_start = match.start()
        # Get the start position of the next runner, or end of block
        if i + 1 < len(runner_starts):
            runner_end = runner_starts[i + 1].start()
        else:
            runner_end = len(main_block_body)

        # Extract this runner's content
        runner = main_block_body[runner_start:runner_end].rstrip()

        # Extract runner name for error reporting
        name_match = re.search(r"^  - name:\s*(.+)", runner, re.MULTILINE)
        runner_name = name_match.group(1).strip() if name_match else "unknown"

        # Extract date from this runner's machine_image
        machine_image_match = re.search(machine_image_pattern, runner)
        if not machine_image_match:
            print(
                f"Error: Could not find machine_image version in runner '{runner_name}'.",
                file=sys.stderr,
            )
            print(f"Runner definition:", file=sys.stderr)
            print(runner, file=sys.stderr)
            print(
                f"\nExpected format: /versions/YYYY.MM.DD in the machine_image field.",
                file=sys.stderr,
            )
            sys.exit(1)

        # Convert date from YYYY.MM.DD to YYYY-MM-DD
        date_str = f"{machine_image_match.group(1)}-{machine_image_match.group(2)}-{machine_image_match.group(3)}"

        # Apply date to labels in this runner
        def append_date_to_label(match: re.Match) -> str:
            ...
            if re.search(r"-\d{4}-\d{2}-\d{2}$", value):
                ...
                new_value = f"{value}-{date_str}"
            return f"{indent}- {new_value}"

        processed_runner = label_pattern.sub(append_date_to_label, runner)
        processed_runners.append(processed_runner)

    # Reassemble the block
    main_block_body_dated = "\n".join(processed_runners)
    ...
    # Strip trailing newlines from new_block, then add proper spacing
    new_block_clean = new_block.rstrip("\n")
    updated_text = (
        prefix_text + "\n\n" + new_block_clean + "\n\n" + suffix_text.lstrip("\n")
    )
    ...
    print(
        f"Updated .cirun.yml for release/{release_version} with new dated block from main branch."
    )

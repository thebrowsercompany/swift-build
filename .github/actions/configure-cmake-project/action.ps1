...
# Note: On GHA, we always build with codeview, so DebugFlags is not a function.
$Assemblers = @{
    Pinned = @{
        Executable       = "clang-cl.exe"
        DriverStyle      = [DriverStyle]::ClangCL
        Flags            = @()
        DebugFlags       = @("-clang:-gcodeview")
...
$Compilers = @{
    MSVC   = @{
        C   = @{
            ...
            Flags            = @("/GS-", "/Gw", "/Gy", "/Oy", "/Oi", "/Zc:preprocessor", "/Zc:inline")
            DebugFlags       = @()
            AssumeFunctional = $false
        }
        CXX = @{
    ...
    Pinned = @{
        ...
        Swift = @{
            ...
            Flags            = @()
            DebugFlags       = @("-g", "-debug-info-format=codeview", "-Xlinker", "/DEBUG")
            AssumeFunctional = $false
        }
    }
...
switch ($OS) {
    'Windows' {
        if ($UseASM) {
            ...
            if ($DebugInfo) {
                ...
                Add-FlagsDefine $Defines CMAKE_ASM_COMPILE_OPTIONS_MSVC_DEBUG_INFORMATION_FORMAT_Embedded $Assembler.DebugFlags
            }
        }
        ...
        if ($UseC) {
            ...
            if ($DebugInfo) {
                Add-FlagsDefine $Defines CMAKE_C_FLAGS $CCompiler.DebugFlags
            }
        }
        ...
        if ($UseCXX) {
            ...
            if ($DebugInfo) {
                Add-FlagsDefine $Defines CMAKE_CXX_FLAGS $CXXCompiler.DebugFlags
            }
        }
        ...
        if ($UseSwift) {
            ...
            if ($DebugInfo) {
                Add-FlagsDefine $Defines CMAKE_Swift_FLAGS $SwiftCompiler.DebugFlags
            } else {
    ...
    'Android' {
        ...
        if ($UseC) {
            ...
            Add-FlagsDefine $Defines CMAKE_C_FLAGS $CCompiler.Flags
            if ($DebugInfo) {
                Add-FlagsDefine $Defines CMAKE_C_FLAGS @('-g', '-gsplit-dwarf')
            }
        }
        ...
        if ($UseCXX) {
            ...
            Add-FlagsDefine $Defines CMAKE_CXX_FLAGS $CXXCompiler.Flags
            if ($DebugInfo) {
                Add-FlagsDefine $Defines CMAKE_CXX_FLAGS @('-g', '-gsplit-dwarf')
            }
        }
        ...
        if ($UseSwift) {
            ...
            if ($DebugInfo) {
                Add-FlagsDefine $Defines CMAKE_Swift_FLAGS @("-g")
            } else {
                Add-FlagsDefine $Defines CMAKE_Swift_FLAGS @("-gnone")
            }
